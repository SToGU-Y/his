<template>
  <div id="recordUpdate">
    <!-- 条件筛选模块 -->
    <PatientSelectCard :deptList="deptList"
    @search="search"></PatientSelectCard>
    <!-- 表格区域 -->
    <div id="table-box">
      <Table border :columns="columns" :data="data">
        <template slot-scope="{ row, index }" slot="action">   
          <Button @click="getRecord(index)" type="primary" size="small" style="margin-right: 5px">病历更改</Button>
          <Button @click="del(index)" type="error" size="small">病历删除</Button>
        </template>
      </Table>
    </div>
    <Modal width="800px" :mask-closable="false" :closable="false" v-model="modal" title="住院病历">
      ...
      <Divider />
      <div id="recordForm">
        <Form ref="update" :model="record" :rules="ruleValidate" label-colon=":" label-position="right" :label-width="100">
          <input disabled type="hidden" v-model="record.admissionId" />
    ...
    <Modal :mask-closable="false" :closable="false" v-model="modal2" title="提示">
        此操作将无法恢复！您确定要删除吗？
        <div slot="footer">
          <Button type="primary" @click="delRecord()">删除</Button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updated(){
        this.record.operator = this.name;
