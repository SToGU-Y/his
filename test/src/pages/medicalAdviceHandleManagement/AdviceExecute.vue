<template>
  <div id="adviceExecute">
        ...
        <Card :bordered="false">
          <div id="cardItem">
            ...
            <Input v-model="conditionForm.name" placeholder="请输入病患名" clearable
              style="width: 150px; margin-right:5px" />
            <Select filterable clearable v-model="conditionForm.status" style="width: 150px; margin-right: 5px">
    ...
    <div id="btn-box">
      ...
      <Button type="primary" v-print="printObj">打印检查单</Button>
    </div>
    <!-- 表格区域 -->
    <div id="table-box">
          ...
          <div id="printMe">
            ...
            <table align="center" border="1" width="700px">
              ...
              <tr v-for="(item,index) in selectiveList" :key="index">
                ...
                <td>{{item.name}}</td>
                <td>{{item.deptName2}}</td>
                <td>{{item.deptName}}</td>
                <td>{{item.wardId}}</td>
                <td>{{item.bid}}</td>
                <td>{{item.madvOperationNo}}</td>
                <td>{{item.madvOperation}}</td>
                <td>{{item.madvOperationType}}</td>
                <td>{{moment(item.madvStartDate).format('YYYY-MM-DD')}}</td>           
              </tr>
            </table>
          </div>
        </div>
